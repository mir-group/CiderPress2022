...
CF = 0.3 * (6 * jnp.pi**2)**(2.0/3)
CFC = 0.3 * (3 * jnp.pi**2)**(2.0/3)

A = 2.74
B = 132
sprefac = 2 * (3 * jnp.pi**2)**(1.0/3)
chiinf = 0.128026
chi = 0.72161
b1c = 0.0285764
b2c = 0.0889
...
def get_pw92term(rs, code):
    ...
    lda = -2*A*(1 + alpha1*rs)*jnp.log(1 + 1/(2.*A*(beta1*jnp.sqrt(rs) \
...
def get_baseline_inf_z(nu, nd, g2, D):
    ...
    tmp = e1lim - e0lim
    vxc = [jnp.zeros((N,2)), jnp.zeros((N,3)),
           jnp.zeros((N,2))]
...
def get_baseline0(rs, zeta, s2):
    ...
    """
    lda = -(b1c/(1 + b2c*jnp.sqrt(rs) + b3c*rs))
    G = get_phi0(zeta)
...
def get_chi_desc(chi):
    ...
    """
    return chi, chi**2, chi**3, chi**4, jnp.ones_like(chi), 2*chi, 3*chi**2, 4*chi**3
...
def get_separate_xef_terms(f, return_deriv=True, cf=2.0):
    ...
    """
    f = f - 1
    fterm0 = jnp.exp(-cf * f**2)
    res = jnp.array([fterm0 * f**i for i in range(5)])
    res[0,:] -= 1
    if return_deriv:
        dres = jnp.array([i * f**(i-1) - 2 * cf * f**(i+1) for i in range(5)])
        dres *= fterm0
        return res, dres
    return res
...
def get_separate_sl_terms(x2, chi, gamma):
    ...
    """
    desc = jnp.zeros((15, x2.shape[0]))
    dx2 = jnp.zeros((15, x2.shape[0]))
    dchi = jnp.zeros((15, x2.shape[0]))
...
def get_separate_xefa_terms(F, chi):
    ...
    x = x[1:4]
    dx = dx[1:4]
    desc = jnp.zeros((13, F.shape[0]))
    df = jnp.zeros((13, F.shape[0]))
    dchi = jnp.zeros((13, F.shape[0]))
    a0, a1, a2, a3, da0, da1, da2, da3 = get_chi_desc(chi)
    ind = 0
    for i in range(3):
        diff = x[i]
...
def get_sl_small(x2, chi, gamma):
    ...
    semi-local descriptors
    """
    desc = jnp.zeros((8, x2.shape[0]))
    dx2 = jnp.zeros((8, x2.shape[0]))
    dchi = jnp.zeros((8, x2.shape[0]))
    u = gamma * x2 / (1 + gamma * x2)
    du = gamma / (1 + gamma * x2)**2
    a0, a1, a2, a3, da0, da1, da2, da3 = get_chi_desc(chi)
    ind = 0
...
def get_xefa_small(F, chi):
    ...
    x = x[1:4]
    dx = dx[1:4]
    desc = jnp.zeros((7, F.shape[0]))
    df = jnp.zeros((7, F.shape[0]))
    dchi = jnp.zeros((7, F.shape[0]))
    a0, a1, a2, a3, da0, da1, da2, da3 = get_chi_desc(chi)
    ind = 0
    for i in range(3):
        diff = x[i]
...
def get_t2(n, zeta, x2):
    t2 = (jnp.pi / 3)**(1./3) / (16 * phi**2) * x2 * n**(1./3)
    dt2dn = t2 / (3 * n)
    dt2dz = -2 * t2 / phi * (1./3) * ((1+zeta)**(-1./3) - (1-zeta)**(-1./3))
    dt2dx2 = (jnp.pi / 3)**(1./3) / (16 * phi**2) * n**(1./3)
...
def get_t2_nsp(n, x2):
    t2 = (jnp.pi / 3)**(1./3) / 16 * x2 * n**(1./3)
    dt2dn = t2 / (3 * n)
    dt2dx2 = (jnp.pi / 3)**(1./3) / 16 * n**(1./3)
...
def get_combined_amix_desc(n, zeta, x2):
    ...
    desc = jnp.zeros((5, n.shape[0]))
    ddescdn = jnp.zeros((5, n.shape[0]))
    ddescdz = jnp.zeros((5, n.shape[0]))
    ddescdx2 = jnp.zeros((5, n.shape[0]))
...
def get_compact_chi_desc(zeta, chi):
    zeros = jnp.zeros_like(zeta)
    zterm = 1 - zeta**8
    dzterm = -8*zeta**7
    desc = jnp.array([1-chi**2, chi**2 * zterm, (chi+chi**2)/2*zterm,
                     chi-chi**3, chi**2-chi**4])
    ddescdz = jnp.array([zeros, chi**2 * dzterm, (chi+chi**2)/2*dzterm,
                        zeros, zeros])
    ddescdchi = jnp.array([-2*chi, 2*chi*zterm, (1+2*chi)/2*zterm,
...
def get_rmn15_desc(n, zeta, x2, chi, F):
    ...
    desc = jnp.zeros((25, n.shape[0]))
    ddescdn = jnp.zeros((25, n.shape[0]))
    ddescdz = jnp.zeros((25, n.shape[0]))
    ddescdx2 = jnp.zeros((25, n.shape[0]))
    ddescdchi = jnp.zeros((25, n.shape[0]))
    ddescdf = jnp.zeros((25, n.shape[0]))
    for i in range(5):
        for j in range(5):
            k = 5*i+j
...
def get_rmn15_desc2(n, zeta, x2, chi, version):
    ...
    if version == 'a': # For F-1
        wterms = jnp.array([w/2+0.5, w**2-1, w**3-w])
        wterms2 = wterms[1:]
    elif version == 'b': # For 1
        wterms = jnp.array([(w-w**2)/2, w**3-w])
        wterms2 = wterms.copy()
    else: # For (f-1)^2 * exp(-(f-1)^2)
        wterms = jnp.array([w, w**2-1, w**3-w])
        wterms2 = wterms[1:]
    wterms *= 1 - ((1+zeta)**(4./3) + (1-zeta)**(4./3) - 2) / (2**(4./3) - 2)
    wterms = jnp.append(wterms, wterms2, axis=0)
    return jnp.concatenate([m1 * wterms, m2 * wterms,
                           m3 * wterms], axis=0)
...
def get_amix_schmidt(n, zeta, x2, chi):
    ...
    """
    zeta = jnp.minimum(zeta, 1-1e-8)
    zeta = jnp.maximum(zeta, -1+1e-8)
    ...
    dt2dz = -2 * t2 / phi * (1./3) * ((1+zeta)**(-1./3) - (1-zeta)**(-1./3))
    dfdz += dfdt2 * dt2dz
    dfdx2 = dfdt2 * (jnp.pi / 3)**(1./3) / (16 * phi**2) * n**(1./3)
...
def get_amix_schmidt2(n, zeta, x2, chi, order=1, mixer='c', zorder=1):
    ...
    dfdx2 = dfdt2 * (jnp.pi / 3)**(1./3) / 16 * n**(1./3)
    # TODO used to be -1/3 below, but I think this was a typo
    dfdn = dfdt2 * (1./3) * t2 / n
    return f, dfdn, dfdz, dfdx2, dfdchi
...
def get_amix_psts(rhou, rhod, g2, Do):
    ...
    exlda /= (rhot)
    u = elim / exlda
    amix = 1 - 1/(1 + A*jnp.log(1 + B*u))
    damix = (A*B)/((1 + B*u)*(1 + A*jnp.log(1 + B*u))**2)
    vxclim[0][:,0] = vxclim[0][:,0] / exlda + elim * dinvldau
    vxclim[0][:,1] = vxclim[0][:,1] / exlda + elim * dinvldad
    vxclim[1] /= exlda[:,jnp.newaxis]
    vxclim[2] /= exlda[:,jnp.newaxis]
    for i in range(3):
        vxclim[i] *= damix[:,jnp.newaxis]
