...
def get_ee_energy_density_outcore2(mol, rdm2, vele_mat, orb_vals):
    ...
    The following script is equivalent and easier to read (but memory-intensive):
...
class CCSDAnalyzer(ElectronAnalyzer):
    """
    ElectronAnalyzer for coupled cluster singles and doubles analysis.
    Also contains a function to compute the perturbative triples correction
    using PySCF.
    ...
    Attributes:
        ...
        ## ALL ATTRIBUTES OF ELECTRON ANALYZER ##
        See ElectronAnalyzer
        NOTE: e_tot contains the correlation energy density but
            NOT the triples excitation, which is stored in
            e_tri if calculated.

        ## IN POST PROCESS ##
        mo_rdm1: 1-electron reduced density matrix in molecular orbital basis
        ao_rdm1/rdm1: 1-electron reduced density matrix in atomic orbital basis
        mo_vele_mat: ao_vele_mat except in molecular orbital basis
        jmat, kmat: Direct and Exchange Coulomb contributions to the
            1-particle Hamiltonian
        ha_total, fx_total: Direct and exchange Coulomb energies of the
            Slater determinant
        ...
        ## COMPUTABLE DATA ## (initialized to None)
        ha_energy_density: Classical Hartree energy density on grid
        ee_energy_density: Coulomb repulsion energy density from 2-RDM
        ee_total: Total electron-electorn Coulomb repulsion energy
        ecorr_dens: CCSD Correlation energy density
        e_tri: Perturbative triples energy correction to the CCSD energy
    """
    ...
    def get_ee_energy_density(self):
        """
        Computes the full electron-electron repulsion energy
        density from the 2-RDM:
        e_{ee} (r_1) = (1/2) \int dr_2 n_2(r_1, r_2) / |r_1 - r_2|,
        where n_2 is the two-particle density (not density matrix).
        """
        if self.ee_energy_density is None:
    ...
    def calc_pert_triples(self):
        """
        Computes the CCSD(T) energy.
        e_tri = E(CCSD(T)) - E(CCSD).
        NOTE: After this call, e_tot still contains E(CCSD)
        without the triples correction. The e_tot + e_tri
        is equal to E(CCSD(T)).
        """
...
class UCCSDAnalyzer(ElectronAnalyzer):
    """
    ElectronAnalyzer for unrestricted coupled cluster singles and doubles analysis.
    Also contains a function to compute the perturbative triples correction
    using PySCF.
    ...
    Attributes:
        ...
        ha_energy_density: Classical Hartree energy density on grid
        ee_energy_density: Coulomb repulsion energy density from 2-RDM
            (_uu, _ud, and _dd spin components are also stored)
        ee_total: Total electron-electorn Coulomb repulsion energy
        ecorr_dens: CCSD Correlation energy density
            (_uu, _ud, and _dd spin components are also stored)
        e_tri: Perturbative triples energy correction to the CCSD energy
    ...
    def get_ee_energy_density(self):
        ...
        e_{ee} (r_1) = (1/2) \int dr_2 n_2(r_1, r_2) / |r_1 - r_2|,
        where n_2 is the two-particle density (not density matrix).
        The returned value is summed over the spins. The _uu, _ud, and
        _dd components are stored as attributes and contain the spin-
        dependent terms.
        """
