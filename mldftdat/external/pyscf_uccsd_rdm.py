...
def _make_rdm2(mycc, d1, d2, with_dm1=True, with_frozen=True, ao_repr=False):
    ...
    h5f = lib.H5TmpFile()
    dm2aa = h5f.create_dataset("dm2aa", (nmoa,nmoa,nmoa,nmoa), dtype=doovv.dtype)
    dm2ab = h5f.create_dataset("dm2ab", (nmoa,nmoa,nmob,nmob), dtype=doovv.dtype)
    dm2bb = h5f.create_dataset("dm2bb", (nmob,nmob,nmob,nmob), dtype=doovv.dtype)

    def assign_vvvv(dm2, dvvvv, nvir1, nvir2, nocc1, nooc2):
        max_memory = max(0, mycc.max_memory - lib.current_memory()[0])
        unit = nvir1**2 * nvir2
        blksize = min(nvir2, max(ccsd.BLKMIN, int(max_memory*.9e6/8/unit)))
        for istep, (p0, p1) in enumerate(lib.prange(0, nvir2, blksize)):
            dm2[nocc1:,nocc1:,nocc2:,nocc2+p0:nocc2+p1] = dvvvv[:,:,:,p0:p1]
...
# dm2aa
    ...
    assign_vvvv(dm2aa, dvvvv, nvira, nvira, nocca, nocca)
...
# dm2bb
    ...
    assign_vvvv(dm2bb, dVVVV, nvirb, nvirb, noccb, noccb)
...
# dm2ab
    ...
    for istep, (p0, p1) in enumerate(lib.prange(0, nvir, blksize)):
        ...
        dm2ab[nocc:,nocc:,nocc:,nocc+p0:nocc+p1] = dvvvv[:,:,:,p0:p1]
    assign_vvvv(dm2ab, dvvVV, nvira, nvirb, nocca, noccb)
