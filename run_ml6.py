# derivative check for HHGGA with CIDER
...
mol = gto.Mole(atom='Co', basis='def2-qzvppd', spin=3, verbose=4)
mol.ecp = 'def2-qzvppd'
mol.build()
hf = scf.UHF(mol)
hf.kernel()
...
ks.conv_tol = 1e-7
ks.kernel()
mf = run_mlscf(mol, 'UKS', SAVE_ROOT, 'CIDER_GLH_TEST')
"""
mf.conv_tol = 1e-8
mf.damp = 4
...
def check_spin():
    dmu = mf.make_rdm1()
    dmd = dmu[[1,0],:,:]
    eu = mf.get_veff(dm=dmu).exc
    ed = mf.get_veff(dm=dmd).exc
    print('SPIN', eu, ed, eu-ed)
