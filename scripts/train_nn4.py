...
X_train, y_train, rho_data_train, wttrain = load_descriptors(dir1, load_wt = True)
X_train2, y_train2, rho_data_train2, wttrain2 = load_descriptors(dir4, load_wt = True)
X_train = np.append(X_train, X_train2[::100], axis=0)
y_train = np.append(y_train, y_train2[::100], axis=0)
wttrain = np.append(wttrain, wttrain2[::100], axis=0)
...
X_test, y_test, rho_data_test, wttest = load_descriptors(dir2, load_wt = True)
X_test2, y_test2, rho_data_test2, wttest2 = load_descriptors(dir2.replace('30_50', '80_100'), load_wt = True)
X_test3, y_test3, rho_data_test3, wttest3 = load_descriptors(dir3, load_wt = True)
...
wttrain = np.append(wttrain, wttest[::200], axis=0)

train_weights = LDA_FACTOR * rho_train**(4.0 / 3) * wttrain

X_train = get_desc2(X_train)
y_train = get_y_from_xed(y_train, rho_data_train)

model = BayesianLinearFeat(48, 8, X_train, y_train, train_weights, order = 2)
#model = PolyAnsatz(NUM)
model.double()
criterion, optimizer = get_traing_obj(model, lr=0.02)
converged = False
i = 0
X_val = get_desc2(X_test[4::100])
y_val = get_y_from_xed(y_test[4::100], rho_data_test[:,4::100])
while not converged:
    loss = train(X_val[::2], y_val[::2], criterion, optimizer, model)
    if i % 100 == 0:
        print('Current Train Loss', loss)
        print(validate(X_val[::2], y_val[::2], criterion, model))
    i += 1
    if loss < 0.0001:
        converged = True
    if i > 3000:
